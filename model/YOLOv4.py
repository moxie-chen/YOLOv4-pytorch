...
from .backbones.swin_transformer import _BuildSwinTransformer
from .layers.global_context_block import ContextBlock2d
from .layers.transformer_layers import C3TR
...
# PANet路径聚合模块：这部分不太好改动，使用复杂路径聚合模块可能会提高准确率但效率可能很低
class PANet(nn.Module):
    def __init__(self, feature_channels):
        ...
        # 这一部分不是PANet模块中的，属于SSP网络后面接的三个卷积网络
        self.transformer_layer = C3TR(feature_channels[2]*2, feature_channels[2] // 2)
    ...
    def forward(self, features):
        ...
        if cfg.TRAIN["Transformer"]:
            downstream_feature5 = self.transformer_layer(features[2])
        else:
...
class YOLOv4(nn.Module):
    def __init__(self, weight_path=None, out_channels=255, resume=False, showatt=False, feature_channels=0):
        ...
        elif cfg.MODEL_TYPE["TYPE"] == "SwinTransformer-YOLOv4":
            # SwinTransformer backbone
            self.backbone, feature_channels = _BuildSwinTransformer()
        else:
